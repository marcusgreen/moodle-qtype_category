...
/**
 * Question type class for the category question type.
 *
 * @package    qtype
 * @subpackage category
...
require_once($CFG->dirroot . '/question/type/category/question.php');


/**
 * The category question type.
 *
 * @copyright  THEYEAR YOURNAME (YOURCONTACTINFO)

 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_category extends question_type {

      /* ties additional table fields to the database */
    public function extra_question_fields() {
        return array('question_category', 'somefieldname','anotherfieldname');
    }
    ...
    public function save_question_options($question) {
        global $DB;
        $options = $DB->get_record('question_category', array('questionid' => $question->id));
        if (!$options) {
        ...
        $DB->update_record('question_category', $options);
 ...
 **/
   ...
   public function initialise_question_answers(question_definition $question, $questiondata,$forceplaintextanswers = true){
    ...
    public function import_from_xml($data, $question, qformat_xml $format, $extra = null) {
        if (!isset($data['@']['type']) || $data['@']['type'] != 'question_category') {
            return false;
        }
    ...
    public function export_to_xml($question, qformat_xml $format, $extra = null) {
        ...
        $gapfillinfo = $pluginmanager->get_plugin_info('question_category');
